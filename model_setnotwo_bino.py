...
def Datagenerate(n, m, bmu, prob, C_inter, Label, rep):
    ...
    Y_ad = Y ** 2 - 2 * Y - 0.4
    p_in_B = pd.DataFrame(Y_ad).applymap(lambda a: norm.cdf(a, loc=5, scale=4))
    pR_generate = np.vectorize(lambda a: np.random.binomial(1, a, 1))

    R = pR_generate(p_in_B.values)
    result = dict(zip(["Y", "Y*R", "Bi", "R", "X"], [Y, Y * R, Bi, R, X]))
    return result
