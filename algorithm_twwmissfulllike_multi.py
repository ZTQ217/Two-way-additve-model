...
def cons(para):
    ...
    P_new = para[range(n+m+1, N_s+n+m+1)]
    C_new = para[range(N_s+n+m+1, 2*N_s+n+m+1)]
...
def gtheta(para):
    ...
    alpha_new = para[range(1, n+1)]
    beta_new = para[range(n+1, n+m+1)]
    P_new = para[range(n+m+1, n+m+N_s+1)]
    C_new = para[range(n+m+N_s+1, n+m+2*N_s+1)]
    sumk = 0
    for k in np.arange(N_s):
        Ck = C_new[k]
        Pk = P_new[k]
        ak = Aij[k]
        theta_all = mu_new + np.dot(alpha_new.reshape(n, 1), Am.reshape(1, m)) + np.dot(An.reshape(n, 1), beta_new.reshape(1, m)) + X * Ck
...
def likelihood(mu_t, alpha_t, beta_t, C_t, P_t):
    ...
    for k in np.arange(N_s):
        ck = C_t[k]
        pk = P_t[k]
        theta_all = mu_t + np.dot(alpha_t.reshape(n, 1), Am.reshape(1, m)) + np.dot(An.reshape(n, 1), beta_t.reshape(1, m)) + X * ck
...
def algorithm(rep):
    ...
    global sample_number, set_of_Y, number_of_Y, Ynozero, Binozero, X, R
    X = Data["X"]
    Y = Data["Y"]
    R = Data["R"]
    Bi = Data["Bi"]
    ...
    print(sample_number)
    Ynozero = Y[np.where(R != 0)]
    Binozero = Bi[np.where(R != 0)]
    set_of_Y = np.unique(Ynozero)
    number_of_Y = [len(np.where(Y == y)[0]) for y in set_of_Y]
    ...
    while diff > 0.05:
        ...
        for k in np.arange(N_s):
            C = C_t[k]
            p = P_t[k]
            theta_t = mu_t + np.dot(alpha_t.reshape(n, 1), Am.reshape(1, m)) + np.dot(An.reshape(n, 1), beta_t.reshape(1, m)) + X * C
        ...
        P_t = para_t[range(n+m+1, n+m+N_s+1)]
        C_t = para_t[range(n+m+N_s+1, n+m+2*N_s+1)]
    ...
    print("prob", prob)
    print("P_t", P_t)
    result = dict(zip(["hat_mu", "hat_alpha", "hat_beta", "hat_P", "hat_C", "sample_number"],
                      [hat_mu, hat_alpha, hat_beta, hat_P, hat_C, sample_number]))
    return result
...
if __name__ == '__main__':
    ...
    plt.subplot(211)
    plt.title(r"$\hat\alpha$_bias_missing={}%_box_n={}_m={}".format(int(n_sample/(n*m)*100), n, m))
    plt.boxplot(pd.DataFrame(alpha_bias), showfliers=False, showmeans=True)
    plt.ylim([-2.5, 2.5])
    plt.xticks([])
    plt.subplot(212)
    plt.title(r"$\alpha^0$ and $\hat\alpha$ mean and median_missing={}%_n={}_m={}".format(int(n_sample/(n*m)*100), n, m))
    ...
    plt.xticks([])
    plt.savefig('./groupGLM_missing={}%_fulllike_alpha_est_n={}_m={}'.format(int(n_sample/(n*m)*100), n, m))
    plt.show()
    plt.close()
    plt.subplot(211)
    plt.title(r"$\hat\beta$_bias_box_missing={}%_n={}_m={}".format(int(n_sample/(n*m)*100), n, m))
    plt.boxplot(pd.DataFrame(beta_bias), showfliers=False, showmeans=True)
    plt.ylim([-2.5, 2.5])
    plt.xticks([])
    plt.subplot(212)
    plt.title(r"$\beta^0$ and $\hat\beta$ mean and median_missing={}%_n={}_m={}".format(int(n_sample/(n*m)*100), n, m))
    ...
    plt.xticks([])
    plt.savefig('./groupGLM_missing={}%_fulllike_beta_est_n={}_m={}'.format(int(n_sample/(n*m)*100), n, m))
    plt.show()
    plt.close()
    pd.DataFrame(alphahat_sd).to_csv("/.groupGLM_missing={}%_fulllike_alphahatsd_n={}_m={}.csv".format(int(n_sample/(n*m)*100), n, m), index=True, header=True)
    pd.DataFrame(alpha_rmse).to_csv("/.groupGLM_missing={}%_fulllike_alpharmse_n={}_m={}.csv".format(int(n_sample/(n*m)*100), n, m), index=True,
                                     header=True)
    pd.DataFrame(betahat_sd).to_csv("/.groupGLM_missing={}%_fulllike_betahatsd_n={}_m={}.csv".format(int(n_sample/(n*m)*100), n, m), index=True, header=True)
    pd.DataFrame(beta_rmse).to_csv("/.groupGLM_missing={}%_fulllike_betarmse_n={}_m={}.csv".format(int(n_sample/(n*m)*100), n, m), index=True,
                                     header=True)
