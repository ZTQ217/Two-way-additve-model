...
def phipr(ob, yob, biob, c, pi):
    theta_newpr = ob + X * c
    Py1newpr = np.exp(theta_newpr)
    Py0newpr = 1 / (1+np.exp(theta_newpr))
    ynewpr = Py1newpr ** yob * (Py0newpr ** biob) * pi
    return ynewpr


def gtheta_gamma(para):
    mu_new = para[0]
    C_new = para[range(1, N_s+1)]
    P_new = para[(N_s+1):len(para)]
    sumk = 0
    pyrenew = np.zeros(sample_number)
    for k in np.arange(N_s):
        Ck = C_new[k]
        Pk = P_new[k]
        ak = Aij[k]
        theta_newpr = mu_new + X * Ck
        theta_new = theta_newpr[R != 0]
        logyi = ak * (Ynozero * theta_new - Binozero * np.log(1 + np.exp(theta_new)))
        Logyi = np.sum(logyi)
        logpk = np.sum(ak) * np.log(Pk)
        loglikeli = Logyi + logpk
        sumk = sumk + loglikeli
        thetamunew = theta_newpr - X * Ck
        thetamunew_ob = thetamunew[R != 0]
        phiij_t = np.vectorize(lambda obb, yyob, bbiob: np.sum(phipr(obb, yyob, bbiob, Ck, Pk)))
        pyrenewk = phiij_t(thetamunew_ob, Ynozero, Binozero)
        pyrenew = pyrenew + pyrenewk
    sumpr = np.sum(pyrenew / Cij)
    result = sumk - sumpr
    return -result
...
def likelihood(mu_t, C_t, P_t):
    logone = 0
    sumpr = np.zeros(sample_number)
    for k in np.arange(N_s):
        ck = C_t[k]
        pk = P_t[k]
        theta_all = mu_t + X * ck
        theta_ob = theta_all[R != 0]
        like_ob = np.exp(theta_ob) ** Ynozero * (1/(1 + np.exp(theta_ob))) ** Binozero
        logone = logone + like_ob * pk
        thetamu = theta_all - X * ck
        thetamu_ob = thetamu[R != 0]
        phiij_t = np.vectorize(lambda obb, yyob, bbiob: np.mean(phipr(obb, yyob, bbiob, ck, pk)))
        sumprk = phiij_t(thetamu_ob, Ynozero, Binozero)
        sumpr = sumpr + sumprk
    result = np.sum(np.log(logone)) - np.sum(np.log(sumpr))
    return result
...
def algorithm(rep):
    ...
    while diff > 0.01:
        ...
        Cij = np.zeros(sample_number)
        for k in np.arange(N_s):
            C = C_t[k]
            p = P_t[k]
            theta_tpr = mu_t + X * C
            theta_t = theta_tpr[R != 0]
            Py1 = np.exp(theta_t) / (1 + np.exp(theta_t))
            Py0 = 1 - Py1
            Pyp = Py1 ** Ynozero * (Py0 ** (Binozero - Ynozero)) * p
            aij.append(Pyp)
            sumak = sumak + Pyp
            thetamut = theta_tpr - X * C
            thetamut_ob = thetamut[R != 0]
            phiij_t = np.vectorize(lambda obb, yyob, bbiob: np.sum(phipr(obb, yyob, bbiob, C, p)))
            Cijk = phiij_t(thetamut_ob, Ynozero, Binozero)
            Cij = Cij + Cijk
        ...
        eq_cons = {'type': 'eq',
                   'fun': cons}
        print(gtheta_gamma(para_0))
        Para_t = minimize(gtheta_gamma, para_0, constraints=eq_cons)
        para_t = Para_t.x
        mu_t = para_t[0]
        print("mu_t =", mu_t)
    ...
    print("prob", prob)
    print("P_t", P_t)
    result = dict(zip(["hat_mu", "hat_P", "hat_C"],
                      [hat_mu, hat_P, hat_C]))
    return result
...
if __name__ == '__main__':
    ...
    initializer()
    p = get_context("spawn").Pool(core, initializer, ())
    N_rep = 3
